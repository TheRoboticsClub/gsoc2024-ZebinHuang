...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(description="Train a BERT model for action classification.")
...
def prepare_data(file_path):
    ...
    labels = pd.factorize(data['action_label'])[0]
...
def main():
    ...
    # Prepare data
    instructions, labels = prepare_data(args.file_path)
    ...
    model = BertForSequenceClassification.from_pretrained(
        args.model_name, num_labels=len(np.unique(labels))
    )
    ...
    # Save the model
    model_save_path = os.path.join(args.output_dir, 'bert_model.pt')
