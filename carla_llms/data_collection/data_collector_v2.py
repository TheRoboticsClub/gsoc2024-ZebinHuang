...
def collect_data_for_episode(vehicle, agent, spectator, world, params, episode_cnt, rgb_cam, seg_cam):
    ...
    while True:
        ...
        process_frame(vehicle, agent, episode_data, frame, rgb_cam, seg_cam, prev_location, world)
        world.tick()
...
def process_frame(vehicle, agent, episode_data, frame, rgb_cam, seg_cam, prev_location, world):
    ...
    if next_wp:
        ...
        episode_data['distance_to_next_wp'].append(distance_to_next_wp)
    else:
        episode_data['distance_to_next_wp'].append(float('inf'))  # if no waypoint found

    # Distance traveled
    distance_traveled = vehicle.get_transform().location.distance(prev_location)
    episode_data['distance_traveled'].append(distance_traveled)
    ...
    episode_data['distance_to_stop_line'].append(distance_to_stop_line)
...
def get_next_waypoint(vehicle, world, distance=1.0):
    ...
    # Get the next waypoint ahead by a specified distance
    next_waypoints = current_waypoint.next(distance)  # returns a list of waypoints
