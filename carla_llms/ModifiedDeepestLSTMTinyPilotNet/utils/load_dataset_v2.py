...
class CARLADataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.one_hot:
            # for fix error with action class
            hlc = torch.clamp(hlc, max=4)
        ...
        # new added distance
        distance = torch.tensor(file['distance_traveled'][idx][0], dtype=torch.float32)
        # we add new command to 50m distance
        distance = torch.clamp(distance / 50.0, 0, 1.0).to(torch.float32)

        return img, sample['measurements'], sample['hlc'], sample['light'], controls, distance
