...
class PilotNetOneHotDistance(nn.Module):
    def __init__(self, image_shape, num_labels, num_hlc, num_light):
        super(PilotNetOneHotDistance, self).__init__()
        self.num_channels = image_shape[2]
        self.cn_1 = nn.Conv2d(self.num_channels, 8, kernel_size=3, stride=2)
        self.relu_1 = nn.ReLU()
        self.cn_2 = nn.Conv2d(8, 8, kernel_size=3, stride=2)
        ...
        self.fc_1_v2 = nn.Linear(8*35*24+1+num_hlc+num_light+1, 50)  # add embedding layer output size
        self.relu_fc_1 = nn.ReLU()
        self.fc_2 = nn.Linear(50, 10)
        self.relu_fc_2 = nn.ReLU()
        self.fc_3 = nn.Linear(10, num_labels)

    def forward(self, img, speed, hlc, light, distance):
        with torch.no_grad():
            out = self.cn_1(img)
            out = self.relu_1(out)
            out = self.cn_2(out)
            ...
            out = torch.cat((out, speed, hlc, light, distance), dim=1)  # Concatenate the high-level commands to the rest of the inputs

        out = self.fc_1_v2(out)
        out = self.relu_fc_1(out)
        out = self.fc_2(out)
        out = self.relu_fc_2(out)
        out = self.fc_3(out)
